...
class TvOnTheAirAdapter(private val items: ArrayList<TvOnTheAirModel>): RecyclerView.Adapter<TvOnTheAirAdapter.TvOnTheAirHolder>() {
    ...
    fun addList(list: List<TvOnTheAirModel>) {
        items.clear()
        items.addAll(list)
    ...
    class TvOnTheAirHolder(val binding: ItemMoviesBinding): RecyclerView.ViewHolder(binding.root)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): TvOnTheAirHolder {
        return TvOnTheAirHolder(
    ...
    override fun onBindViewHolder(holder: TvOnTheAirHolder, position: Int) {
    ...
    interface OnItemClickCallback {
        fun onClickItem(data: TvOnTheAirModel)
    }
}