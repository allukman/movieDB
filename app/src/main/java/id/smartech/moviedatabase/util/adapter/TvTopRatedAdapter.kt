...
class TvTopRatedAdapter(private val items: ArrayList<TvTopRatedModel>): RecyclerView.Adapter<TvTopRatedAdapter.TopRatedHolder>() {
    ...
    fun addList(list: List<TvTopRatedModel>) {
        items.clear()
        items.addAll(list)
        notifyDataSetChanged()
    }

    class TopRatedHolder(val binding: ItemMoviesBinding): RecyclerView.ViewHolder(binding.root)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): TopRatedHolder {
        return TopRatedHolder(
    ...
    interface OnItemClickCallback {
        fun onClickItem(data: TvTopRatedModel)
    }
}